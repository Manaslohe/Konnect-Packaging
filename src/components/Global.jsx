...
const Global = () => {
  ...
  const FootprintCard = ({ title, description, icon }) => (
    <div className="box-border relative bg-white rounded-2xl w-full mx-auto overflow-hidden shadow-none border-2 border-gray-300 max-sm:rounded-xl" style={{fontFamily: 'Krona One, sans-serif'}}>
      ...
      {/* Title section */}
      <div className="relative px-4 py-2 bg-white text-left max-md:px-3 max-md:py-2 max-sm:px-2 max-sm:py-1">
        <h3 className="text-base font-bold text-black leading-tight max-md:text-sm max-sm:text-xs" style={{fontFamily: 'Krona One, sans-serif'}}>
          {title}
        </h3>
      </div>
      
      {/* Description section */}
      <div className="relative px-4 py-4 bg-white min-h-[120px] flex items-start justify-start max-md:px-3 max-md:py-3 max-md:min-h-[100px] max-sm:px-2 max-sm:py-2 max-sm:min-h-[80px]">
        <p className="text-gray-600 text-xs text-left leading-relaxed font-normal max-md:text-[0.7rem] max-sm:text-[0.6rem]" style={{fontFamily: 'Krona One, sans-serif'}}>
          {description}
        </p>
      </div>
    </div>
