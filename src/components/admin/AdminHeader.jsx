...
const SIDEBAR_BG = 'bg-gradient-to-r from-[#E9C77F] to-[#FBE6B7]';
  ...
  return (
    <header
      className={`flex flex-row items-center justify-between gap-2 px-3 md:px-6 py-3 md:py-4 
        ${SIDEBAR_BG} border-b border-[#E9C77F] sticky top-0 z-20 shadow-sm`}
    >
      {/* Hamburger for mobile */}
      <button
        className="md:hidden flex items-center justify-center w-10 h-10 rounded-full bg-black text-[#E9C77F] font-bold text-xl shadow-md border-2 border-[#E9C77F] hover:scale-105 transition"
      ...
      <div
        ...
        style={{ fontFamily: 'Krona One, sans-serif', color: '#111' }}
      >
        Admin Portal
      </div>
      <div className="relative flex items-center gap-2 md:gap-4" ref={dropdownRef}>
        <button
          className="flex items-center justify-center w-10 h-10 rounded-full bg-black text-[#E9C77F] font-bold text-lg shadow-md border-2 border-[#E9C77F] hover:scale-105 transition"
        ...
        {/* Dropdown */}
        {dropdownOpen && (
          <div className="absolute right-0 top-12 mt-2 w-40 bg-white rounded-xl shadow-lg border border-[#E9C77F] z-30 animate-fade-in">
            <button
